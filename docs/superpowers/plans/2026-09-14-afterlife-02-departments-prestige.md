...
- iOS (spec §16): the Capacitor iOS project lives in `ios/`, deployment target iOS 15, portrait only, `appId` `com.afterlifebureaucracy.game`; all fixed UI respects `env(safe-area-inset-top/bottom)`. This machine is Windows: the iOS project is generated and configured here, but the Xcode build is unverified until run on a Mac — say so in reports and docs.
...
| `src/sim/simulate.ts`, `src/sim/run.ts`, `src/sim/pacing.test.ts` | Balance simulator + pacing assertions |
| `ios/` (generated), `capacitor.config.ts`, `src/ui/theme.css`, `docs/ios-build.md` | iOS shell, safe-area insets |
...
### Task 9: iOS shell and safe-area layout

**Files:**
- Create: `ios/` (generated by Capacitor), `docs/ios-build.md`
- Modify: `package.json` (add `@capacitor/ios`, scripts `cap:sync:ios`, `cap:open:ios`), `capacitor.config.ts`, `.gitignore`, `src/ui/theme.css`, `ios/App/App/Info.plist`, `ios/App/App.xcodeproj/project.pbxproj` (deployment target only, via `sed`)
- Test: `src/ui/App.test.tsx` (safe-area class assertion)

**Interfaces:**
- Produces: `npx cap sync ios` succeeds on Windows (it copies the web bundle and writes `Podfile`; CocoaPods install and the Xcode build are Mac-only and remain unverified here). `capacitor.config.ts` gains `ios: { contentInset: 'never', backgroundColor: '#1B1915' }`. `Info.plist` locks portrait for iPhone and iPad and sets `UIRequiresFullScreen` true. Deployment target `IPHONEOS_DEPLOYMENT_TARGET = 15.0`. The app shell gets `padding-top: env(safe-area-inset-top)` on `.screen` and the memo ticker/tab bar already use `env(safe-area-inset-bottom)`.
...
Expected: FAIL — `.app` lacks class `safe-area`.

- [ ] **Step 3: Add the iOS platform**

Run:
```bash
npm install @capacitor/ios@^7.6.9
```
...
```
Add scripts to `package.json`: `"cap:sync:ios": "npm run build && cap sync ios"`, `"cap:open:ios": "cap open ios"`.
Run:
```bash
npm run build && npx cap add ios && npx cap sync ios
```
Expected: `ios/App` created; sync reports the same plugins as Android (`@capacitor/app`, `@capacitor/preferences`). `cap sync ios` on Windows prints a warning that `pod install` was skipped — that is expected; note it in the report.
...
- [ ] **Step 4: Portrait lock and deployment target**

In `ios/App/App/Info.plist` set (replace the existing arrays if present):
```xml
...
<true/>
```
Set the deployment target in `ios/App/App.xcodeproj/project.pbxproj` (Git Bash):
```bash
...
```
Expected: count ≥ 2. Also set `platform :ios, '15.0'` in `ios/App/Podfile` if Capacitor generated a different version.

- [ ] **Step 5: Safe-area CSS**

In `src/ui/App.tsx` change the shell root to `<div className="app safe-area">`. Append to `src/ui/theme.css`:
```css
.safe-area .screen { padding-top: calc(12px + env(safe-area-inset-top)); }
.safe-area .modal-backdrop { padding-top: calc(20px + env(safe-area-inset-top)); padding-bottom: calc(20px + env(safe-area-inset-bottom)); }
.safe-area .memo-ticker { bottom: calc(var(--tabbar-h) + env(safe-area-inset-bottom)); }
```
(`.tabbar` already pads its bottom with `env(safe-area-inset-bottom)`; the ticker sits above that padding.)

- [ ] **Step 6: Write docs/ios-build.md**
...
- [ ] **Step 7: Run tests and verify the sync artefacts**

Run: `npm test` and `npx cap sync ios`
Expected: all tests PASS; sync succeeds (pod install skipped on Windows). `git status` must show no `ios/App/App/public` or `Pods` files staged.

- [ ] **Step 8: Commit**

```bash
git add package.json package-lock.json capacitor.config.ts .gitignore ios docs/ios-build.md src/ui
git commit -m "feat(ios): Capacitor iOS shell, portrait lock, iOS 15 target, safe-area layout"
git push origin main
```
State in the report that the Xcode build is unverified on this machine.
...
- §16 iOS readiness: Task 9 (project generated and configured; build verified only on a Mac).
