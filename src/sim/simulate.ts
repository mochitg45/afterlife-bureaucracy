...
import { computeRates, staffBulkCost, upgradeCost, upgradeLevel } from '../engine/economy';
...
import { rollover, claimDaily, isDone } from '../engine/dailies';
...
export function simulate(opts: SimOptions, content: Content): SimResult {
  ...
  for (let day = 1; day <= opts.days; day++) {
    ...
    for (let s = 0; s < opts.sessionsPerDay; s++) {
      ...
      state = spendVouchers(state, content, rates.kcPerSec);
      vouchersSpentToday += Math.max(0, beforePull - state.vouchers);
      state = equipBest(state, content);
      const claimed = claimDone(state, content, rates.kcPerSec);
      state = claimed.state;
